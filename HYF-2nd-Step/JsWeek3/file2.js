...
const LANGUAGE = ["English", "Japanese", "Arabic", "...etc"];
// let text,textMode;
        // text = TITLEs.map(
        //     (d) => {
        //         textMode = document.createTextNode(d);
        //     }
        // );        console.log(text)
function unOrderedList(ul) {
    ul = document.createElement("ul");
    function theListItSelf(li) {
        function u(textMode, id, index) {
            textMode = 0;
            id = 0;
            index = 0;
            for (textMode, id, index; (textMode, id, index) < CONAN_IDs.length; textMode++ , id++ , index++) {
                li = document.createElement("li");
                li.setAttribute("id", CONAN_IDs[index]);
                ul.appendChild(li);

                let h4 = document.createElement("h4");
                textMode = document.createTextNode(TITLEs[index]);
                h4.appendChild(textMode);

                let p = document.createElement("p");
                textMode = document.createTextNode(CHARACTERS.join(", "));
                p.appendChild(textMode);

                let span = document.createElement("span");
                textMode = document.createTextNode(LANGUAGE.join(", "));
                span.appendChild(textMode);

                let img = document.createElement("img");
                img.setAttribute("src", SOURCES[index]);
                img.setAttribute("alt", TITLEs[index]);
        ...
        u();
// OR Like So ... 
        let content = CONAN_IDs.reduce(
            (textMode, id, index, array) => {
                li = document.createElement("li");
                li.setAttribute("id", CONAN_IDs[index]);
                ul.appendChild(li);
                ...
                li.appendChild(span);
                li.appendChild(p);
                // theListItSelf = document.createElement("li");
                // theListItSelf.setAttribute("id", b);
                // unOrderedList.appendChild(theListItSelf);
                // a = document.createTextNode(TITLEs[c]);
                // theListItSelf.appendChild(a)
                // console.log(b)
                // console.log(a)
            }
            , "");
    ...
    const allTogether = {
        ...
        function s(textMode, id, index, li) {
            ...
            for (textMode, id, index; (textMode, id, index) < allTogether.CONAN_IDs.length; textMode++ , id++ , index++) {
                li = document.createElement("li");
                li.setAttribute("id", allTogether.CONAN_IDs);
                ul.appendChild(li);
