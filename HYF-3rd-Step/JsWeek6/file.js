...
function fourArgumentsFunc(start,end,oddDivisible,fiveDivisible) {
    ...
    // let checkThreeArray = filteringThreeRange.map(item => {
    //     return (item % 3 === 0) ? oddDivisible(filteringThreeRange) : false;
    // })
    // let checkFiveArray = filteringFiveRange.map(item => {
    //     return (item % 5 === 0) ? oddDivisible(filteringFiveRange) : false;
    // })

    // let mappedThree = range.map(item => {
    //      ? oddDivisible(item) : undefined;
    // })
    // range.map(item => {
    //      ? fiveDivisible(item) : undefined;
    // })
    //     start.concat(end);
    // (range instanceof Array) ? range : range.split(",");
}
...
// >>>> Q.3 
// ......The Answers Are After The Functions
...
// console.log(comprator())
//  console.log(BONUS.sort(comprator))
// console.log(newSotedBONUS)
